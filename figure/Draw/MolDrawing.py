...
class MolDrawing(object):
    ...
    def _drawLabel(self, label, pos, baseOffset, font, color=None, scale=120, **kwargs):
        color = color or self.drawingOptions.defaultColor
        x1 = pos[0] + baseOffset[0]  # fixed
        y1 = pos[1] + baseOffset[1]  # fixed
        labelSize = list(self.canvas.addCanvasText(label, (x1, y1), font, color, **kwargs))  # fixed
        # labelSize[0] = 0.065 if labelSize[0] <= 0.065 else labelSize[0]
        # labelSize[0] = 0.800 if labelSize[0] >= 0.800 else labelSize[0]
        # print(labelSize)
        labelSize = tuple([0.076 * scale, 0.070 * scale, 0])
        return labelSize
                ...
                if includeAtomNumbers:
                    ...
                    if deg == 0:
                        if periodicTable.GetElementSymbol(atom.GetAtomicNum()) in ('O', 'S', 'Se', 'Te', 'F',
                                                                                   'Cl', 'Br', 'I', 'At'):
                            symbol = r'$\mathregular{%s%s%s%s%s%s}$' % (hs, isotope, base, chg, rad, mapNum)
                        else:
                            symbol = r'$\mathregular{%s%s%s%s%s%s}$' % (isotope, base, hs, chg, rad, mapNum)
                    elif deg > 1 or nbrSum[0] < 1:
                        symbol = r'$\mathregular{%s%s%s%s%s%s}$' % (isotope, base, hs, chg, rad, mapNum)
                        baseOffset[0] = 0.5 - (isotopeLength + len(base) / 2.) / symbolLength
                    else:
                        symbol = r'$\mathregular{%s%s%s%s%s%s}$' % (rad, chg, hs, isotope, base, mapNum)
                        # baseOffset = 3 - 0.5 + (mapNumLength + len(base) / 2.) / symbolLength  # fixed
                        baseOffset[0] = - 0.5 + (mapNumLength + len(base) / 2.) / symbolLength  # fixed
                    if deg == 1:
                        if abs(nbrSum[1]) > 1:
                    ...
                    if orient == 'W':
                        ...
                        baseOffset[1] += 3
                        baseOffset[0] += 3 * (symbolLength - 1) / 2
                    elif orient == 'S':
                        baseOffset[1] -= 3
                        baseOffset[0] += 3 * (symbolLength - 1) / 2
                    elif orient == 'C':
